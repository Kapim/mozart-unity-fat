...
One shared control model for portals and MATs:

- **Grip (hand trigger)** — the object rigidly follows that controller, so moving and rotating the
  controller moves and rotates the object together. Right hand wins when both grip. This is the
  **only** way to move an object.
- **Index trigger** — selects/points only, never moves. Pilot users found trigger-driven dragging
  too easy to set off by accident, so ray-grab on the trigger was removed.
- **Thumbsticks** — per-axis scale on the object's own local axes, portals only (`AllowScale`);
  MATs are pose-only. Right stick X/Y = width/height, left stick Y = depth.

After a gesture the change is pushed to ARCOR2 through the object's `IObjectServerBinding`.

Objects are **auto-selected when added**: `GameManager` remembers what it asked the server to
create and, once the server echoes it back and it is spawned, enters the matching edit mode and
selects it — so a freshly added portal or MAT is immediately manipulable without aiming at it.
